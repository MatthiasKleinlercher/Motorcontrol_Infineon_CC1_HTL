 ...
 *******************************************************************************/
uint8_t lut_index = 0;
const float32_t angle[6][2] = //rotor | stator
{
    { 11.0f*PI/6.0f, PI/6.0f },
    { PI/6.0f,       PI/2.0f },
    { 3.0f*PI/6.0f,  5.0f*PI/6.0f },
    { 5.0f*PI/6.0f,  7.0f*PI/6.0f },
    { 7.0f*PI/6.0f,  9.0f*PI/6.0f },
    { 9.0f*PI/6.0f,  11.0f*PI/6.0f }
};
...
void hall_interrupt_handler(void)
{
    ...
    if (intr_status1 || intr_status2 || intr_status3)
    {
		...
		if (CY_TCPWM_COUNTER_STATUS_COUNTER_RUNNING & Cy_TCPWM_Counter_GetStatus(Speed_Measurment_Counter_HW, Speed_Measurment_Counter_NUM))
		{
			...
			if (foc_state != FOC_RUNNING_OPEN_LOOP) w_e = TWO_M_PI * freq/ ((float32_t)(countValueSpeMeaCou-prevCount)* (float32_t)LUT_SIZE) * 4.0;
...
int main(void)
{
    ...
    {		
        ...
        /* UART */
        char print_Array[25];
        float32_t data_Array[6] = {I_q, I_q_ref, Speed_Control.kp, w_e, w_e_ref,w_e_sensored};
        print_Array[0] = 0xAA;
